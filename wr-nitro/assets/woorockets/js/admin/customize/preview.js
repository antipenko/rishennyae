...
(function($) {
	...
	api( 'general_line_color', function( value ) {
		value.bind( function( to ) {
			update_live_preview( [ { 'classes': '.preview_line_color_normal', 'color': to,  'type': 'border-color' }, { 'classes': '.preview_line_color_bg', 'color': to,  'type': 'background-color' } ] );
		} );
	} );
	...
	api( 'w_style', function( value ) {
		value.bind( function( to ) {
			...
			if ( ( to == '1' || to == '2' ) && widget_bg == true ) {
				widget.css({ 'padding': '20px', 'background': bg_color });
			...
			if ( to == '4' ) {
				...
				if ( widget_bg == true ) {
					widget.css({ 'background-color': bg_color, 'padding': '20px 10px', 'border': 'none' });
					widget.addClass('overlay_bg');
	...
	api( 'wr_page_layout_sidebar_width', function( value ) {
		value.bind( function( to ) {
			...
			if ( 'right-sidebar' == layoutValue ) {
				$( '.page .primary-sidebar' ).css( 'width', to + 'px' );
				$( '.page .main-content' ).css( 'width', 'calc(100% - ' + to + 'px)' );
			} else if ( 'left-sidebar' == layoutValue ) {
				$( '.page .primary-sidebar' ).css({
					'width': to + 'px',
					'right': 'calc(100% - ' + to + 'px)'
				});
