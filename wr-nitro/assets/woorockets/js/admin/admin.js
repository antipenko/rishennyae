	...
	function init_demos() {
		...
		$( '#demos .install-sample, #demos .uninstall-sample' ).click( function( event ) {
			...
			import_sample_data = $.proxy( function( id ) {
				...
				$( '#nitro-manipulate-sample-data' ).off( 'load' ).load( $.proxy( function( event ) {
					...
					if ( response.responseJSON.data && response.responseJSON.data.refresh_nonce ) {
						$.ajax( {
							...
							dataType: 'json',
							data: {
								nonce: response.responseJSON.data.refresh_nonce
							},
							complete: function( response ) {
								if ( response.responseJSON && response.responseJSON.success ) {
				...
				}, this ) ).attr( 'src', wr_nitro_admin.install_sample_url + '&package=' + id + '&step=3&nonce=' + wr_nitro_admin.install_sample_nonce );
			}, this ),
							...
							complete: function( response ) {
								...
								if ( !response.responseJSON || !response.responseJSON.success ) {
									...
									if ( response.responseJSON && response.responseJSON.data ) {
										...
										if ( message ) {
											$( '#wr-install-sample-data-required-plugins .wr-status' ).append( message[ 1 ] );
										} else {
											$( '#wr-install-sample-data-required-plugins .wr-status' ).append( response.responseJSON.data );
										}
									} else {
										$( '#wr-install-sample-data-required-plugins .wr-status' ).append( wr_nitro_admin.install_plugin_failed.replace( '%s', data.required_plugins[ i ] ) );
									}
								}
					...
					$.ajax( {
						...
						complete: function( response ) {
							if ( !response.responseJSON || !response.responseJSON.data ) {
								...
								$( '#TB_ajaxContent' ).on( 'click', '#confirm-sample-data-installation', function( event ) {
