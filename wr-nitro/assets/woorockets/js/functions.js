	 ...
	 */
	 function HB_Element_Cart() {
	 	$( '.hb-cart.sidebar' ).click( function() {
			...
			$( 'body > .hb-cart-outer' ).html( '<span class="close-cart"><span></span></span>' + cart_content );
			...
			switch ( position ) {
				...
				if ( animation == 'sidebar-push' || animation == 'sidebar-fall-down' || animation == 'sidebar-fall-up' )
					...
					if ( animation == 'sidebar-push' || animation == 'sidebar-fall-down' || animation == 'sidebar-fall-up' )
						...
						if ( animation == 'sidebar-slide-in-on-top' || animation == 'sidebar-push' || animation == 'sidebar-fall-down' ){ console.log( 'fasdfas' );
						shop_item.css( {
							'visibility': 'visible',
...
function close_cart( _this ) {
	...
	_this.animate( {
		...
		$( 'body' ).on( 'click', '.action-item .edit-btn-action', function() {
			var _this = $( this );
			var parent = _this.closest( '.mini_cart_item' );
			var cart_item_key = parent.attr( 'data-key' );
			var cart_item_number = parent.find( '.action-item .edit-number' ).val();
			var position = _this.closest( '.hb-minicart' ).attr( 'data-slidebar-position' );
		 ...
		 */
		 ...
		 function add_to_cart_ajax( _this ) {
		 	...
		 	if ( data_post.search( 'add-to-cart=' ) != -1 ) {
		 		...
		 		var url_request = data_post.replace( 'add-to-cart', 'wr-custom-add-to-cart' ) + '&action=wr_product_add&_nonce=' + _nonce_wr_nitro;

		 		$.ajax( {
		 			type: 'POST',
		 			url: WRAjaxURL,
		 			data: WR_Parse_Url_To_Array( url_request ),
		 			success: function( val ) {
		 				if ( val.status == 'true' ) {
		 					$( 'body > .wr-notice-cart-outer' ).remove();
		 ...
		 */
		 ...
		 $( 'form.cart.variations_form' ).on( 'show_variation', function( event, variation, purchasable ) {
		 	event.preventDefault();

		 	var _this = $( this );

		 	if ( purchasable ) {
		 		_this.find( '.single_buy_now' ).removeAttr( 'disabled' ).removeAttr( 'title' );
		 		_this.find( '.single_buy_now' ).removeAttr( 'disabled' ).removeAttr( 'title' );

				// Enable floating button
				$( '.floating-add-to-cart button' ).removeAttr( 'disabled' );
			} else {
				_this.find( '.single_buy_now' ).attr( 'disabled', 'disabled' );

				// Disnable floating button
				$( '.floating-add-to-cart button' ).attr( 'disabled', 'disabled' );
			}
		} );

		 $( 'form.cart.variations_form' ).on( 'hide_variation', function( event ) {
		 	event.preventDefault();
		 ...
		 $( 'form.cart .single_buy_now' ).click( function( e ) {
		 	e.preventDefault();
		 	e.stopPropagation();
		 	...
		 	if ( data_post.search( 'add-to-cart=' ) != -1 ) {
		 		...
		 		/* Replace key for custom ajax */
		 		var url_request = data_post.replace( 'add-to-cart', 'product_id' ) + '&action=wr_quickbuy&_nonce=' + _nonce_wr_nitro;
...
function resize_menu_list( list_element, width_parent ) {
	 ...
	 */
	 function WR_Sidebar_Sticky() {
	 	...
	 	var frame_handler = _.debounce( function() {
	 		...
	 		if( height_sidebar_inner > height_compare || height_sidebar_inner >= height_sidebar ) {
	 			sidebar.removeClass( 'fixed-bottom fixed-top' );
	 			return;
	 		}
	 ...
	 * [ Init Owl Carousel ] - - - - - - - - - - - - - - - - - - - - - - - -
	 */
	 function WR_Carousel() {
		// Check if OwlCarousel plugin for jQuery is loaded before setting up carousel.
		if ( typeof $.fn.owlCarousel == 'undefined' ) {
			return setTimeout( WR_Carousel, 100 );
		}

		var el = $( '.wr-nitro-carousel' );

		el.each( function( i, val ) {
			var _this = $( this );

			if ( _this.hasClass( 'exclude-carousel' ) ) {
				return;
			}

			var _option = _this.data( 'owl-options' );

			if ( _option !== undefined ) {
				var _autoplay = ( 'true' == _option.autoplay ) ? true : false, _autotime = ( _option.autoplayTimeout ) ? _option.autoplayTimeout : '5000', _items = _option.items, _nav = ( 'true' == _option.nav ) ? true : false, _dots = ( 'true' == _option.dots ) ? true : false, _pause = ( 'true' == _option.autoplayHoverPause ) ? true : false, _tablet = _option.tablet, _mobile = _option.mobile, _rtl = ( 'true' == _option.rtl ) ? true : false, _loop = ( _option.loop ) ? _option.loop : true, _autoHeight = ( 'true' == _option.autoHeight ) ? true : false, _animateIn = ( _option.animateIn ) ? _option.animateIn : '', _animateOut = ( _option.animateOut ) ? _option.animateOut : '';
	 		...
	 		if ( _option !== undefined ) {
	 			...
	 			$( val ).countdown( {
	 				...
	 				render: function( data ) {
	 					$( this.el ).html( "<div class='pr'><span class='db color-primary'>" + this.leadingZeros( data.days, 2 ) + "</span><span class='db tu ls-1 color-dark'>days</span></div><div class='pr'><span class='db color-primary'>" + this.leadingZeros( data.hours, 2 ) + "</span><span class='db tu ls-1 color-dark'>hrs</span></div><div class='pr'><span class='db color-primary'>" + this.leadingZeros( data.min, 2 ) + "</span><span class='db tu ls-1 color-dark'>mins</span></div><div class='pr'><span class='db color-primary'>" + this.leadingZeros( data.sec, 2 ) + "</span><span class='db tu ls-1 color-dark'>secs</span></div>" );
	 				}
	 			} );
	 		}
	 	} );
	 ...
	 */
	 function WC_Floating_Addcart() {
	 	...
	 	$( window ).load( function() {
	 		...
	 		var footer = $( '.footer' ), cart_button = $( '.p-single-action .single_add_to_cart_button' ), cartTop = cart_button.offset().top + cart_button.height(), footer_height = footer.height(), window_height = $( window ).height(), document_height = $( document ).height(), width_content_broswer = $( window ).width(), offset = ( ( width_content_broswer > 1024 ) ? parseInt( WR_Data_Js[ 'offset' ] ) : 0 ) + 10;
	 		...
	 		var addcart_frame_handler = function() {
	 			...
	 			if ( scrollTop + window_height < document_height - footer_height ) {
	 				cart.css( {
	 					'bottom': offset + 'px',
	 					'top': 'auto'
	 				} );
	 ...
	 */
	 function WC_Products_Filter() {
	 	var result      = $( '#woof_results_by_ajax' ),
	 	wc_switch   = $( '.wc-switch' ),
	 	show        = $( '.woof_show_auto_form, .woof_hide_auto_form' );

	 	if ( ! result.length ) {
	 		return;
	 	}
		// Move switch layout
		wc_switch.appendTo( result );

		// Move show/hide auto filter
		show.appendTo( result );

		$( document ).ajaxComplete( function( event, xhr, settings ) {
			var data_request = ( typeof settings.data != 'undefined' ) ? settings.data : '';

			if ( data_request.search( 'action=woof_draw_product' ) != -1 ) {
				$( '.woocommerce-result-count,.woocommerce-ordering' ).wrapAll('<div class="wc-filter-result fc jcfe aic" />');
			}

		});

		result.closest('#shop-main').children('.fc.jcfe').removeClass('fc');
	 ...
	 */
	 function WR_Shortcode_Member() {
	 	...
	 	memberAvatar.on( 'click', function() {
	 		...
	 		var grandParent = $( this ).closest( '.nitro-member' );
	 		var member_item = grandParent.find( '.member' );
	 		var currentItem = member_item.index( parent ) + 1;
	 		var countItem = member_item.length;
	 		var grandParentWidth = grandParent.width();
	 		var parentWidth = parent.width();
	 		var column = parseInt( grandParentWidth / parentWidth );
	 		var memberContent = $( this ).next();
	 ...
	 */
	 ...
	 $( document ).ready( function() {
		...
		function overlayOpen(){
			header.classList.add('wrapper-head');
		}
		function overlayClose(){
			header.classList.remove('wrapper-head');
		}

		var modalOverlay = document.querySelector("#modal-overlay");
		var modalConslutation = document.querySelector("#modal-conslutation");
		var modalComment = document.querySelector("#modal-comments");
		var closeButtonConslutation = document.querySelector("#close-button-consl");
		var closeButtonComment = document.querySelector("#close-button-comment");
		var openButtonConslutation = document.querySelector("#advocat-free-conslutation");
		var openButtonComment = document.querySelector("#advocat-comments");

		var header = document.querySelector('#wrapper-head');

		closeButtonConslutation.addEventListener("click", function() {
			modalConslutation.classList.toggle("closed");
			modalOverlay.classList.toggle("closed");
			overlayClose();
		});

		openButtonConslutation.addEventListener("click", function() {
			modalConslutation.classList.toggle("closed");
			modalOverlay.classList.toggle("closed");
			overlayOpen();
		});

		closeButtonComment.addEventListener("click", function() {
			modalComment.classList.toggle("closed");
			modalOverlay.classList.toggle("closed");
			overlayClose();
		});

		openButtonComment.addEventListener("click", function() {
			modalComment.classList.toggle("closed");
			modalOverlay.classList.toggle("closed");
			overlayOpen();
		});
		...
		modalOverlay.addEventListener("click", function(){
			...
			modalComment.classList.add("closed");
			modalConslutation.classList.add("closed");
			overlayClose();
		});
