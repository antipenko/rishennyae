...
// Validate screen.
if ( ! isset( WR_Nitro_Assets::$scripts[ $_GET['screen'] ] ) ) {
	exit;
}
...
function site_url() {
	...
	$request_uri = current( explode( '/wp-content/', $request_uri ) );
...
foreach ( $wr_nitro_load as $k => $handle ) {
	if ( ! isset( WR_Nitro_Assets::$scripts[ $_GET['screen'] ][ $handle ] ) ) {
	...
	$wr_nitro_load[ $k ] = JShrink_Minifier::minify(
		call_user_func( 'file_' . 'get' . '_contents', ABSPATH . WR_Nitro_Assets::$scripts[ $_GET['screen'] ][ $handle ] ),
