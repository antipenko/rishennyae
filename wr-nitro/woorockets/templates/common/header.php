...
$wr_is_revslider_activated   = ( file_exists( ABSPATH . 'wp-content/plugins/revslider/revslider.php' ) && in_array( 'revslider/revslider.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) ? true : false;
$wr_is_woocommerce_activated = ( file_exists( ABSPATH . 'wp-content/plugins/woocommerce/woocommerce.php' ) && in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) ? true : false;
$wr_is_wrls_activated        = ( $wr_is_woocommerce_activated && file_exists( ABSPATH . 'wp-content/plugins/wr-live-search/main.php' ) && in_array( 'wr-live-search/main.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) ? true : false;
$wr_is_wpml_activated        = ( file_exists( ABSPATH . 'wp-content/plugins/sitepress-multilingual-cms/sitepress.php' ) && in_array( 'sitepress-multilingual-cms/sitepress.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) ? true : false;
		...
		foreach ( $wr_nitro_header_data['rows'] as $key => $val ) {
						...
						foreach ( $wr_hb_col['items'] as $key_item => $val_item ) {
							...
							switch ( $wr_hb_item['_rel'] ) {
								...
								case 'search':
									...
									if( $wr_hb_item['centerElement'] && $wr_hb_settings['type'] == 'horizontal' && ! $wr_is_mobile && ! $wr_center_element_once ) {
									...
									$wr_nitro_header_html[] = '
										...
										$wr_nitro_header_html[] = '
												...
												<span class="open ' . ( ( $wr_hb_item['layout'] == 'full-screen' || $wr_hb_item['layout'] == 'topbar' ) ? 'show-full-screen' : '' ) . '" data-layout="' . esc_attr( $wr_hb_item['layout'] ) . '" data-background-style="' . esc_attr( $wr_hb_item['searchStyle'] ) . '"><i class="fa fa-search"></i></span>
											</div>';
									$wr_nitro_header_html[] = '
										</div>';
								...
								case 'menu':
									...
									if ( ! $wr_is_mobile ) {
										...
										if( $wr_hb_item['textSettings'] ) {
											...
											if( $wr_item_css ) {
												$wr_nitro_header_css[] = '.header .hb-s' . ( $key + 1 ) . 'i' . ( $key_item + 1 ) .' .site-navigator{' . $wr_item_css . '}';
												$wr_nitro_header_css[] = '.header .hb-s' . ( $key + 1 ) . 'i' . ( $key_item + 1 ) .' .site-navigator a{' . $wr_item_css . '}';
											}
										}
								...
								case 'logo':
										...
										$wr_nitro_header_html[] = '<div class="content-logo"><a href="' . esc_url( home_url( '/' ) ) . '">';
											...
											// Logo image
												...
												if( $wr_hb_row['sticky'] == 1 && $wr_hb_item['logoImageSticky'] ) {
													...
													$wr_logo_retina = 'http://wp.rishennyae.com/wp-content/uploads/logo.png';
													...
													$wr_nitro_header_html[] = $wr_logo_retina ? '<img width="' . absint( $wr_hb_item['style']['maxWidth'] ) . '" height="10" class="logo-retina-show" style="width: 200px;max-width: 250px;" src="' . $wr_logo_retina . '" ' . $wr_alt . '/>' : NULL;
												}
											}
									...
									if( $wr_item_css ) {
										$wr_nitro_header_css[] = '.hb-s' . ( $key + 1 ) . 'i' . ( $key_item + 1 ) . ' a{display:block; ' . $wr_item_css . '}';
									}
			...
			if( $wr_hb_row['sticky'] == 1 ) {
				...
				if ( intval( $wr_hb_row['heightSticky'] ) > 0 ) {
					...
					$wr_nitro_header_css[] = '
						.header .sticky-row-scroll .section-' . ( $key + 1 ) . ' img{ max-height: ' . $wr_heightSticky . 'px }';
				}
				...
				$wr_nitro_header_css[] = '.header .sticky-row-scroll .section-' . ( $key + 1 ) .' .element-item,
						.header .sticky-row-scroll .section-' . ( $key + 1 ) .' .element-item.hb-logo a { margin-top:0; margin-bottom:0; padding-top:' . ( $wr_has_el_wpml ? '5px' : 0 ) . '; padding-bottom:' . ( $wr_has_el_wpml ? '5px' : 0 ) . '}';
				...
				if( $wr_hb_row['textColorSticky'] ) {
					...
					.header .sticky-row-scroll .section-' . ( $key + 1 ) .' .hb-cart .number-price .amount,
