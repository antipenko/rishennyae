...
class WR_Nitro_Megamenu_Walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if( isset( $args->is_mobile ) && $args->is_mobile ) {
			...
			$item_output =
				'<div class="item-link-outer"><a ' . $attributes . '><span class="menu_title">' . esc_attr( $item->title ) . '</span></a>' . ( $this->has_children ? '<i class="has-children-mobile fa fa-angle-down"></i>' : '' ) . '</div>';
		} else {
