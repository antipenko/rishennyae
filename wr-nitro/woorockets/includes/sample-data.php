...
class WR_Nitro_Sample_Data {
	 ...
	 * @var  string
	 */
	protected static $server = 'http://www.woorockets.com/files/sampledata/sample-data.json';
	...
	protected static function download_sample_package( $package ) {
		...
		$path = wp_upload_dir();
		$path = $path['basedir'] . '/' . $package['id'] . '.zip';
		...
		if ( ! self::download( $package['download'], $path ) ) {
			wp_send_json_error( __( 'Failed to download the selected sample data package.', 'wr-nitro' ) );
		}
	 ...
	 *
	 * @param   array    $package    Sample package data.
	 * @param   boolean  $no_import  Whether or not to import sample data to database.
	 *
	 * @return  void
	 */
	protected static function import_sample_data( $package, $no_import = false ) {
		// Get WordPress file system object.
		global $wp_filesystem;

		// Generate path to downloaded sample data package.
		$upload_dir = wp_upload_dir();
		$path       = $upload_dir['basedir'] . '/' . $package['id'];
		...
		if ( current( array_slice( explode( '/', str_replace( '\\', '/', get_template_directory() ) ), -1 ) ) != $id ) {
			wp_send_json_error(
				sprintf(
			...
			if ( $tables && count( $tables ) ) {
				foreach ( $tables as $table ) {
					...
					foreach ( $results as $result ) {
						...
						// Store insert query.
						$backups[] = "INSERT IGNORE INTO `{$table}` {$keys} VALUES {$values};";
					}
				}
			}
		}
		...
		if ( count( $backups ) ) {
			...
			$path = $upload_dir['basedir'] . '/' . $package['id'] . '/backup_' . date( 'YmdHis' ) . '.sql';

			// Write database backup to file
			$wp_filesystem->put_contents( $path, implode( "\n", $backups ) );
		}
		...
		// Define options that values need to be reserved.
		...
		foreach ( $xml->xpath( '//product/extension' ) as $plugin ) {
			...
			if ( $queries && count( $queries ) ) {
				foreach ( $queries as $query ) {
					...
					// Truncate options table instead of drop.
					if ( false !== strpos( $query, 'DROP TABLE' ) && false !== strpos( $query, "{$table_prefix}options" ) ) {
						$query = "DELETE FROM {$table_prefix}options WHERE option_name NOT IN ( '"
							. implode( "', '", $reserved_options ) . "' );";
					}
					...
					elseif ( false !== strpos( $query, 'INSERT INTO' ) ) {
						...
						elseif ( false !== strpos( $query, "{$table_prefix}options" ) ) {
							...
							// Remove Google Analytics ID.
							if ( 'theme_mods_' . current( array_slice( explode( '/', str_replace( '\\', '/', get_template_directory() ) ), -1 ) ) == $option ) {
		...
		if ( $session_tokens ) {
			update_user_meta( $current_user->ID, 'session_tokens', $session_tokens );
		}
		...
		// Store installed sample data package and its dependencies.
	...
	protected static function restore_backup_data( $package ) {
		...
		$backup = wp_upload_dir();
		$backup = glob( $backup['basedir'] . '/' . $package['id'] . '/backup_*.sql' );
		...
		if ( count( $backup ) ) {
			...
			// Import backup data.
			foreach ( explode( ";\n", $wp_filesystem->get_contents( $backup ) ) as $query ) {
				$wpdb->query( "{$query};" );
			}
	...
	protected static function convert_special_chars( $string )
	{
		...
		$search  = array( chr( 145 ), chr( 146 ), chr( 147 ), chr( 148 ), chr( 151 ), "\r", "\n" );
		$replace = array( "'"       , "'"       , '"'       , '"'       , '-'       , ' ' , ' '  );
