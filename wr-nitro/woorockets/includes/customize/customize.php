...
class WR_Nitro_Customize {
	...
	public static function initialize( $wp_customize ) {
		...
		if ( $wp_customize ) {
			...
			add_action( 'customize_controls_print_footer_scripts', array( __CLASS__, 'print_dependencies' ) );
	...
	/**
	 * Print controls dependencies data.
	 *
	 * @return  void
	 */
	public static function print_dependencies() {
				...
				setTimeout( function() {
					...
					for ( var i = 0, n = nodes.childNodes.length; i < n; i++ ) {
						if ( nodes.childNodes[i].textContent.indexOf( 'Nitro' ) > -1 ) {
							...
							nodes.replaceChild( elm, nodes.childNodes[i] ); break;
						}
					}
				}, 500 );
