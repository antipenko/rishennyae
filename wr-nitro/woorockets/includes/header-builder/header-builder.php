...
class WR_Nitro_Header_Builder {
	...
	public static function creat_header_default() {
		$data = new WP_Query( array( 'post_type' => 'header_builder', 'posts_per_page' => 1 ) );
	...
	public static function render_columns( $column, $post_id ) {
		...
		switch ( $column ) {
			case 'status' :
				...
				if( function_exists( 'icl_object_id' ) && isset( $_GET['lang'] ) && $_GET['lang'] != 'all' ) {
					global $wpdb;
	...
	protected static function recursive_check_name( $name_neutral, $number_name_neutral = 0 ) {
		...
		// Verify header name.
		$data = new WP_Query( array( 'post_type' => 'header_builder', 'post_title' => $name_header ) );
	...
	public static function enqueue_admin_assets() {
		...
		if( $post_type_current == 'header_builder' ) {
			...
			if( function_exists( 'icl_object_id' ) && ( $pagenow == 'post-new.php' || $pagenow == 'post.php' ) ) {
